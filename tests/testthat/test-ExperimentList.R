...
test_that("Constructor works (SingleCellExperiment)", {
  ...
  #with annotations
  edata = data.frame('ID' = 1:3, row.names = letters[1:3])
  expect_error(SingleCellExperimentList(sce_empty_list, experimentData = edata))
  ...
  rownames(edata) = NULL
  expect_error(SingleCellExperimentList(sce_empty_list, experimentData = edata))
  expect_true(validObject(SingleCellExperimentList(sce_empty_list, experimentData = edata, check.names = FALSE)))

  rownames(edata) = LETTERS[1:2]
  expect_true(validObject(SingleCellExperimentList(sce_empty_list, experimentData = edata)))
